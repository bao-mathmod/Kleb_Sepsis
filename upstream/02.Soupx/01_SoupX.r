...
# USER SETTINGS
# =========================
base_dir <- "/mnt/18T/chibao/pbmc/data_2"
...
# SCTransform options
use_glmGamPoi <- TRUE   # set TRUE if you have glmGamPoi installed (faster SCT on big data)
...
# =========================
# SECTION 2: Build Seurat + QC + SCT clustering (for clusters/UMAP only)
# =========================
build_seurat_for_soupx <- function(toc, sample_id, out_dir) {
  ...
  # minimal QC filter for stability (still permissive)
  save_qc_plot(
    seu,
  ...
  # SCTransform instead of log-normalize
  if (use_glmGamPoi) {
...
# =========================
# SECTION 3: Make SoupX channel + attach clusters + UMAP
# =========================
build_soup_channel <- function(tod, toc, seu) {

  keep_cells <- colnames(seu)

  # Subset toc to match Seurat-good cells
  toc_sub <- toc[, keep_cells, drop = FALSE]

  # Ensure tod has same gene rows as toc_sub (common + safe)
  tod_sub <- tod[rownames(toc_sub), , drop = FALSE]

  # Build SoupChannel using the subset toc
  sc <- SoupChannel(tod = tod_sub, toc = toc_sub)

  # Now clusters covers ALL cells in sc$toc
  clusters <- setNames(as.character(seu$seurat_clusters), colnames(seu))

  # Optional debug: ensure perfect match
  if (!setequal(colnames(sc$toc), names(clusters))) {
    missing <- setdiff(colnames(sc$toc), names(clusters))
    stop("Clusters missing for ", length(missing), " cells. Example: ", missing[1])
  }

  sc <- setClusters(sc, clusters)

  # UMAP embedding (subset to keep_cells for guaranteed alignment)
  umap <- Embeddings(seu, "umap")[keep_cells, , drop = FALSE]
  sc <- setDR(sc, umap)
...
# =========================
# SECTION 4: Estimate contamination + correct counts
# =========================
estimate_and_correct <- function(sc, round_to_int = TRUE) {
  # DO NOT call estimateSoup() here if SoupChannel already did it
...
# =========================
# SECTION 5: Save outputs (rho stats + plots + corrected Seurat)
# =========================
save_outputs <- function(sample_id, out_dir, seu_pre, sc, corrected) {
  # Save rho per cell
barcodes <- colnames(sc$toc)

# rho values
rho <- sc$metaData$rho
if (is.null(rho)) stop("rho is NULL: autoEstCont() may not have set it.")

# ensure rho aligns to barcodes
if (length(rho) != length(barcodes)) {
  # try align by rownames in metaData if available
  if (!is.null(rownames(sc$metaData)) && all(barcodes %in% rownames(sc$metaData))) {
    rho <- sc$metaData[barcodes, "rho"]
  } else {
    stop("rho length (", length(rho), ") != #barcodes (", length(barcodes),
         "). Can't safely align.")
  }
}
...
rho_df <- data.frame(
  ...
  # Return numeric summary (example: median rho)
  data.frame(
    sample_id = sample_id,
    ...
    rho_p95 = as.numeric(quantile(rho, 0.95, na.rm = TRUE)),
    stringsAsFactors = FALSE
  )
}

# =========================
# MAIN: One sample (now modular)
# =========================
run_soupx_one_sample <- function(sample_dir, sample_id, out_base_dir) {
  ...
  message("Class of soup object: ", paste(class(soup), collapse = ", "))

  est <- run_step("Estimate contamination + adjust counts", {
    estimate_and_correct(soup, round_to_int = TRUE)
  })
  ...
  # Print a quick numeric checkpoint (useful when watching logs)
  message("✅ ", sample_id, ": n_cells_corrected=", smry$n_cells_corrected,
...
for (sd in sample_dirs) {
  ...
  sm <- tryCatch(
    ...
    error = function(e) {
      message("⚠️ Skipping sample ", sample_id, " due to error.")
      NULL
    }
  )
